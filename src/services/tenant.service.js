...
const getTenantDetails = async ({ currentUser }) => {
    ...
    if (!tenant) {
        const missingDetails =
            buildTenantDetailsWhenProfileMissing(currentUser);
...
const ensureKycFilePresent = (kycDocLocalPath) => {
    if (!kycDocLocalPath) {
        logger.error(
            `KYC file is required for KYC verification, provided kycDocLocalPath: ${kycDocLocalPath}`,
        );
...
const ensurePropertyOwnedByLandlord = (property) => {
    if (!property) {
        logger.error(
            `Property not found or does not belong to landlord, propertyId: ${property?._id}`,
        );
        throw new ApiError(
            404,
...
const ensureTenantProfileExistsForUser = (tenant) => {
    if (!tenant) {
        logger.error(
            `Tenant profile not found for user, userId: ${tenant?.userId}`,
        );
        throw new ApiError(
            404,
...
const ensurePropertyExists = (property) => {
    if (!property) {
        logger.error(
            `Property not found at tenantService.ensurePropertyExists, propertyId: ${property?._id}`,
        );
...
const ensureTenantNotAlreadyAssigned = (tenant) => {
    if (tenant.isActive && tenant.propertyId) {
        logger.error(
            `Tenant is already assigned to a property, tenantId: ${tenant._id}, propertyId: ${tenant.propertyId}`,
        );
